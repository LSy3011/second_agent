...
Second Agent 解决的是“企业知识助手如何拥有长期记忆、工具调用能力和可维护任务流程”的问题。

它和 Paper Assistant 的分工是：

- Paper Assistant：沉淀论文、技术报告和内部文档，提供可检索的领域知识服务。
- Second Agent：沉淀用户上下文、任务偏好和历史过程，负责工具选择、Skill 调用和个性化推理。
...
- Neo4j 图谱关系可写入和查询。
- `knowledge-workflow`、`paper-digest`、`career-advancement`、`resume-polisher`、`interview-coach` Skill 可识别。
...
- `knowledge-workflow`：企业知识工作流规划，适合研发知识助手、技术调研和内部文档问答。
- `paper-digest`：论文解读、方法对比和技术 claim 提取。
- `career-advancement`：职业发展建议，作为个人成长场景的验证样例。
- `resume-polisher`：项目经历表达优化，作为可选任务包。
- `interview-coach`：面试问答准备，作为可选任务包。

## 项目表述

> 基于 Mem0 + Qdrant + Neo4j 构建本地化长期记忆 Agent Runtime，支持向量记忆检索、图谱关系写入、Skill 工作流调用和推理轨迹持久化；面向企业知识助手与研发知识工作流场景，解决本地 BGE-M3 embedding 维度适配、Neo4j/APOC 部署、LangChain 依赖兼容等工程问题，并在服务器完成健康检查、Agent 推理、图谱写入和 Skill 调用验证。
